...
def num_to_words(n):
    ...
    if n in list(numbers.keys()):
        return numbers[n]
    elif 10 < n < 20:
        return numbers[divmod(n, 10)[1]] + 'teen'
    elif 30 <= n <= 90 and n % 10 == 0:
        return numbers[n/10] + 'ty'
    else:
        tens, ones = divmod(n, 10)
        return '{}-{}'.format(num_to_words(tens*10), num_to_words(ones))
...
class NounConjunction():
    def __init__(self, nouns):
        self.nouns = nouns

    def realize(self):
        if len(self.nouns) == 1:
            return self.nouns[0].realize()
        if len(self.nouns) == 2:
            return "{} and {}".format(self.nouns[0].realize(), self.nouns[1].realize())
        return ", ".join([n.realize() for n in self.nouns[:-1]]) + ", and " + self.nouns[-1].realize()

class NounPhrase():

    def __init__(self, word, determiner=None, adjectives=None, owners=None, prepositional_phrases=None, number=1):
        self.base_word = word
    ...
    def add_determiner(self, determiner):
        if isinstance(determiner, Determiner) or isinstance(determiner, Word):
            ...
            raise Exception(
                "A determiner must be an instance of Determiner or Word. Type of determiner given: " + str(type(determiner)))
    ...
    def wrap_noun_in_np(subject):
        ...
        if not isinstance(subject, Noun) and not isinstance(subject, NounPhrase):
            raise Exception("The subject of the Clause must either be a Noun or a NounPhrase. Type of " +
                            str(subject) + ": " + str(type(subject)))
        np_subject = subject
    ...
    def realize(self):
        ...
        if len(self.adjectives) >= 2:
            for adj in self.adjectives[1:-1]:
                # TODO AUGH, really? Am I REALLY adding a comma by just
                # appending it here? That'll never work, try again
                np.append(", "+str(adj))
            np.append("and")
            np.append(str(self.adjectives[-1]))

        if(self.number > 1):
            np.append(num_to_words(self.number))
            if isinstance(self.base_word, Word):
                np.append(str(self.base_word.plural))
            else:
                np.append(self.base_word.realize())
        else:
            if isinstance(self.base_word, Word):
                np.append(str(self.base_word))
            else:
